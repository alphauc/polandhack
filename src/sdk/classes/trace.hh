...
namespace sdk {
	...
	struct ray_t {
		...
		void initialize(const vec3& vecStart, const vec3& vecEnd) {
	...
	class trace : public base_interface {
	public:
		int GetPointContents(const vec3& vecAbsPosition, int contentsMask = MASK_ALL, IHandleEntity** ppEntity = NULL) {
			//typedef int (__thiscall* fn)(void*, const vec3&, int, IHandleEntity**);
			return get_vfunc<0, int>(this, vecAbsPosition, contentsMask, ppEntity);
		}

		void TraceRay(const ray_t& ray, unsigned int fMask, i_trace_filter* filter, trace_t* trace) {
			//typedef void (__thiscall* fn)(void*, const Ray_t&, unsigned int, ITraceFilter*, trace_t*);
			return get_vfunc<5, void>(this, ray, fMask, filter, trace);
		}

		void ClipRayToEntity(const ray_t& ray, unsigned int fMask, base_entity* pEnt, trace_t* pTrace) {
			//typedef void (__thiscall* OriginalFn)(void*, const Ray_t&, unsigned int, base_entity*, trace_t*);
			return get_vfunc<3, void>(this, ray, fMask, pEnt, pTrace);
		}
	};
}
